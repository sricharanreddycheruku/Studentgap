...
const navigation = [
  ...
  { to: '/whatsapp', label: 'Setup', icon: MessageCircleMore }
];
    ...
    <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/95 backdrop-blur-md">
      <div className="mx-auto flex w-full max-w-[1480px] items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
        <Link to="/" className="flex min-w-0 items-center gap-2.5 shrink-0">
          <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#11233f] text-white">
            <Activity size={16} />
          </span>
          <span className="hidden sm:block">
            <span className="block text-base font-black text-[#11233f] leading-tight tracking-tight">ClassPulse</span>
            <span className="block text-[10px] font-semibold text-slate-400 leading-tight uppercase tracking-wider">Learning diagnostics</span>
          </span>
        </Link>
        ...
        <nav className="flex items-center gap-0.5" aria-label="Main navigation">
              ...
              className={({ isActive }) =>
                `flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold transition-all ${
                  isActive
                    ...
                    : 'text-slate-500 hover:bg-slate-100 hover:text-[#11233f]'
                }`
              }
            >
              <Icon size={15} />
          ...
          {localStorage.getItem('classpulse-teacher') && (
            <button
              ...
              onClick={signOut}
              title="Sign out"
              className="ml-1 grid h-8 w-8 place-items-center rounded-lg text-slate-400 transition hover:bg-rose-50 hover:text-rose-500"
            >
              <LogOut size={15} />
            </button>
          )}
